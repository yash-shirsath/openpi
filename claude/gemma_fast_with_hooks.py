...
# Hook configuration - set to True to capture attention inputs
...
def _save_attention_inputs(x, positions, attn_mask, kv_cache, decode):
    """Save attention inputs to disk using jax.debug.callback"""
    global _global_step_counter
    
    def save_to_disk(x, positions, attn_mask, kv_cache_data, decode, step):
        # Create output directory
        output_dir = "/root/openpi/claude/attention_captures"
        ...
        data_dict = {
            ...
            'metadata': {
                ...
                'seq_len': x.shape[1],
                'features': x.shape[2] if len(x.shape) > 2 else None,
            }
        }
        
        # Add KV cache data if present
        if kv_cache_data is not None:
            data_dict['kv_cache_idx'] = np.array(kv_cache_data[0])
            data_dict['kv_cache_k'] = np.array(kv_cache_data[1])
            data_dict['kv_cache_v'] = np.array(kv_cache_data[2])
        else:
            data_dict['kv_cache_idx'] = None
            data_dict['kv_cache_k'] = None
            data_dict['kv_cache_v'] = None
        
        # Save to compressed numpy file
        filename = f"attention_inputs_{int(timestamp)}_{int(step)}.npz"
        filepath = os.path.join(output_dir, filename)
        np.savez_compressed(filepath, **data_dict)
        print(f"Saved attention inputs to: {filepath}")
    
    # Convert kv_cache to serializable format
    kv_cache_data = kv_cache if kv_cache is not None else None
    
    # Use JAX debug callback to save on host
    jax.debug.callback(save_to_disk, x, positions, attn_mask, kv_cache_data, decode, _global_step_counter)
...
class Attention(nn.Module):
    ...
    def __call__(self, x, positions, attn_mask, kv_cache, decode, deterministic=True):  # noqa: FBT002
        # Capture inputs if hook is enabled
        if CAPTURE_ATTENTION_INPUTS:
            _save_attention_inputs(x, positions, attn_mask, kv_cache, decode)
